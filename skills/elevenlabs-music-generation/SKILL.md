...
- **Scope of bash usage**: the skill only invokes `runcomfy ` — `npm` / `npx` lines are one-time operator setup, not commands the skill executes per call.
...
- [`ai-music`](https://www.skills.sh/agentspace-so/runcomfy-agent-skills/ai-music) — intent router across ElevenLabs Music, ACE Step, and audio edit endpoints
