---
name: typegpu
description: TypeGPU and WebGPU adapter patterns for HyperFrames. Use when creating GPU-rendered compositions with TypeGPU, raw WebGPU, WGSL fragment shaders, compute pipelines, liquid glass effects, particle systems, or any canvas layer driven by navigator.gpu that responds to HyperFrames hf-seek events.
---
...
- Full-screen fragment shaders, compute pipelines, and particle systems driven by composition time.
- Liquid glass, frosted blur, and video-backed distortion when the render browser supports WebGPU + html-in-canvas.
- GPU overlays (`alphaMode: 'premultiplied'`) on top of HTML or video layers.

## Avoid

- Using `Math.random()`, `requestAnimationFrame`, or `performance.now()` for animation time.
- Registering GSAP tweens after the first `await` in WebGPU initialization.
- Assuming `copyExternalImageToTexture` works for video in headless render mode without a fallback texture path.
- Skipping `device.queue.onSubmittedWorkDone()` before frame capture in render mode.
...
After editing a TypeGPU / WebGPU composition:
...
- HyperFrames adapter source: `packages/core/src/runtime/adapters/typegpu.ts`.
- Upstream adapter notes: `skills/hyperframes-animation/adapters/typegpu.md` in [heygen-com/hyperframes](https://github.com/heygen-com/hyperframes).
