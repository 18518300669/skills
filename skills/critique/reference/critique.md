...
2. **Compute the slug.** From the repository root, run:
   ```bash
...
**AI Slop Detection (CRITICAL)**: Does this look like every other AI-generated interface? Load and apply `skills/impeccable/SKILL.md` (shared design laws, absolute bans, register references). Check for AI color palette, gradient text, dark glows, glassmorphism, hero metric layouts, identical card grids, generic fonts, and all other tells. **The test**: If someone said "AI made this," would you believe them immediately?
...
Prefer the upstream **Impeccable** CLI when it is available in the environment:
...
**Browser visualization**: **required** when browser automation tools are available AND the target is a viewable page AND `npx impeccable live` works. The `[Human]` overlay tab is the user-facing deliverable when that stack is available. Skip only if the target is not a viewable page (CSS-only file, non-browser target).
...
**Without `npx impeccable`**: Perform a structured manual pass over the same markup and styles: grep and read files for patterns forbidden in `skills/impeccable/SKILL.md` (absolute bans and AI slop test), cross-check `skills/web-design-guidelines/SKILL.md`, and summarize limitations honestly in the **Deterministic scan** section (manual review, not CLI).

Return: CLI findings (JSON) or manual scan notes, browser console findings (if applicable), and any false positives noted.
...
- **Suggested follow-up**: Name which project-local skill or reference file is the best next step (see list below).

**Project-local skills and references you may recommend** (read and follow the matching `SKILL.md` or `reference/*.md`):

- `skills/polish/SKILL.md` — final UI pass
- `skills/impeccable/SKILL.md` and `skills/impeccable/reference/*.md` — craft, shape, audit, adapt, animate, clarify, etc.
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `PRODUCT.md` (or optional `DESIGN.md` / `.agents/context/` per `skills/impeccable`) contains audience or brand context from an `impeccable teach`-style workflow, also generate 1-2 project-specific personas from that info.
...
Once the report above is finalized, write it to `.impeccable/critique/` so the user can refer back, and so `skills/polish/SKILL.md` workflows can pick up the priority issues (see persistence hooks in `skills/polish/reference/polish.md`).
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command from the repository root:
   ```bash
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node skills/critique/scripts/critique-storage.mjs write <slug> <body-file>
   ```
...
**After presenting findings**, use targeted questions based on what was actually found. Keep questions anchored in the report. These answers will shape the action plan.
...
List recommended next steps in priority order, based on the user's answers. Each item should name a concrete project-local skill path (for example `skills/impeccable/reference/polish.md` after implementation work, or `skills/impeccable/reference/audit.md` for technical checks):

1. **`skills/...`**: Brief description of what to fix (specific context from critique findings)
2. **`skills/...`**: Brief description (specific context)
...

**Rules for recommendations**:
- Only recommend skills from the project-local list in **Priority Issues** above (or closely related references under `skills/impeccable/reference/`).
- Order by the user's stated priorities first, then by impact
- Each item's description should carry enough context that the next pass knows what to focus on
- Map each Priority Issue to the appropriate skill or reference
- Skip entries that would address zero issues
- If the user chose a limited scope, only include items within that scope
- If the user marked areas as off-limits, exclude work that would touch those areas
- End with `skills/polish/SKILL.md` as the final step if any UI fixes were recommended

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run this critique workflow after fixes to see your score improve.
