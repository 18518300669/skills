...
- **Both exist**: Ask the user Ask which file to refresh. Skip the one the user doesn't want changed.
...
Ask the user Ask only about what you couldn't infer from the codebase.
...
If the signal is genuinely split (e.g. a product with a big marketing landing), Ask the user Ask which register describes the **primary** surface. The register can be overridden per task later, but PRODUCT.md carries one default.
...
Optionally Ask the user Ask whether they'd like a brief summary of PRODUCT.md appended to AGENTS.md for easier agent reference. If yes, append a short **Design Context** pointer section there.
