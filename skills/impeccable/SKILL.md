...
**This repository:** Run helper scripts from the repository root, for example `node skills/impeccable/scripts/load-context.mjs`. Documentation that says `impeccable <subcommand>` means: follow the matching procedure under `skills/impeccable/reference/<subcommand>.md` together with this file. The optional upstream CLI (`npx impeccable`) is not required when working from these project-local files.
